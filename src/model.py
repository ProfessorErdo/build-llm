...
class PositionalEncoding(nn.Module): 
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None: 
        ...
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model)) # 1 x d_model / 2
...
class LayerNormalization(nn.Module): 
    ...
    def __init__(self, eps: float = 10**-6) -> None: 
        super().__init__()
        self.eps = eps
        self.alpha = nn.Parameter(torch.ones(1)) # multiplined
...
class MultiHeadAttentionBlock(nn.Module): 
    ...
    def forward(self, q, k, v, mask): 
        query = self.w_q(q) # (batch, seq_len, d_model) -> (batch, seql_len, d_model)
        key = self.w_k(k) # (batch, seq_len, d_model) -> (batch, seql_len, d_model)
        value = self.w_v(v) # (batch, seq_len, d_model) -> (batch, seql_len, d_model)
        ...
        # (batch, seq_len, d_model) -> (batc, seq_len, d_model)
        return self.w_o(x)
...
class ProjectionLayer(nn.Moddule): 
